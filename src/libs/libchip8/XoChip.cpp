...
void XoChip::save_vx_to_vy(int x, int y) {
	m_mnemomicFormat = "(X) SAVE V%4$01X-V%5$01X";
	auto step = x > y ? -1 : +1;
	auto address = m_i;
	auto ongoing = true;
	do
...
void XoChip::load_vx_to_vy(int x, int y) {
	m_mnemomicFormat = "(X) LOAD V%4$01X-V%5$01X";
	auto step = x > y ? -1 : +1;
	auto address = m_i;
	auto ongoing = true;
	do {
